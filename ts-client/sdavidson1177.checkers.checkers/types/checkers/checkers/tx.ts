...
export class MsgClientImpl implements Msg {
  ...
  CreatePost(request: MsgCreatePost): Promise<MsgCreatePostResponse> {
    ...
    const promise = this.rpc.request("sdavidson1177.checkers.checkers.Msg", "CreatePost", data);
    return promise.then((data) => MsgCreatePostResponse.decode(new _m0.Reader(data)));
  }

  CreateGame(request: MsgCreateGame): Promise<MsgCreateGameResponse> {
    const data = MsgCreateGame.encode(request).finish();
    const promise = this.rpc.request("sdavidson1177.checkers.checkers.Msg", "CreateGame", data);
